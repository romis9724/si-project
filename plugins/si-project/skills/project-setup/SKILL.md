...
발주처·계약·MM·검수·운영 SLA 등 행정 정보는 본 단계에서 받지 않는다 — 첫 doc/bundle 호출 시점에 `.claude/project-context.json`에 수집.
...
**Branch 전략**: project-context.json이 있고 `contract.acceptance ∈ {단계별, 마일스톤}`이면 `GIT_FLOW_STYLE = "git-flow"`, 그 외 `"trunk-based"`. init 시점에는 보통 컨텍스트가 없어 기본 trunk-based.
