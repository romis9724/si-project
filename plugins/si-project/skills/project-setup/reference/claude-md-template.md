...
## Plugin 동작 플래그 (v2.3.1)

본 섹션은 si-project 스킬들이 동작 시 참조하는 환경 변수. 사용자가 직접 편집해 동작 조정 가능.

### `REVIEWER_SUBAGENT` — 도메인 검토 subagent 호출 정책 (v2.2 / v2.3.1)
- `off` — 검토자 subagent 호출 안 함 (체크리스트는 가이드용으로만)
- `critical-only` — opus 3개(requirements / software-architecture / security-definition)만 호출 (**기본값**)
- `on` — 10개 핵심 산출물 모두 호출 (sonnet 7개 포함)
- 호출 1회당 토큰 비용 발생. 마일스톤 일괄 생성 시 비용 ×산출물 수

`REVIEWER_SUBAGENT: critical-only`

### `GH_CLI_PATH` — GitHub CLI 실행 파일 경로 (project-inbox에서 사용)
- 비어 있으면 PATH에서 `gh` 검색
- PATH 미반영 환경(Windows 등)에서는 절대 경로 명시 (예: `C:/Program Files/GitHub CLI/gh.exe`)
- gh 미설치·미인증·폐쇄망 시 project-inbox는 GitHub 섹션 자동 스킵

`GH_CLI_PATH:`

## 구현 단계 subagent 활용 패턴

산출물 작성 후 코드 구현 단계에서 Agent tool 활용 패턴은 플러그인 reference 참조:
`{플러그인_경로}/reference/subagent-patterns.md`
